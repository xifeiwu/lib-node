...
export async function testRunScriptExportInCP() {
  ...
  const result = await runScriptInCP(scriptPath, {
    cpWrapperOptions: {runTargetScriptOptions: {runExportedFunc: true, funcParams: [10]}},
  });
  logColorful({}, result);
}
...
export async function passFuncNameAndParams() {
  const scriptPath = path.join(__dirname, 'scripts/test.ts');
  const result = await runScriptInCP(scriptPath, {
    cpWrapperOptions: {runTargetScriptOptions: {runExportedFunc: true, funcName: 'add1', funcParams: [10]}},
  });
  logColorful({}, result);
}
