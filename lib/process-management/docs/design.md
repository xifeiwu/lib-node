...
Daemon 在 `~/.daemon/{cpId}/{pid}.sock` 启动一个流式 Unix socket server（`net.createServer`），stdout/stderr 数据实时广播给所有连接的客户端。
...
stdout/stderr 分别写入 `~/.daemon/{cpId}/{pid}.out` 和 `~/.daemon/{cpId}/{pid}.error`。用 `fs.createWriteStream` + `pipe`。
...
每个 CpManager spawn 后将运行信息写入 `~/.daemon/{cpId}/pid-info.json`：
...
流程：
1. `scanAllPidInfoRecords()` 扫描所有 `~/.daemon/{cpId}/pid-info.json`，找到 `status='running'` 的记录
