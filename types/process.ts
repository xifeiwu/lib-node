export interface ProcessInfo {
  ...
  cpu: number;
  /**
   * Resident set size. From `getProcessInfo` / `ps`: typically KiB. From `getProcessInfoByInst`: bytes
   * (`process.memoryUsage().rss()`).
   */
  rss: number;
  /** Virtual memory size. From `getProcessInfo` / `ps`: typically KiB. From `getProcessInfoByInst`: may be 0. */
  vsize: number;
  /** Human-readable RSS: `byteToWord` applied to byte length (see {@link rss} for how raw values map to bytes). */
  rssWord: string;
