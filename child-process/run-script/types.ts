...
export namespace CP {
  ...
  //   // | 'echo-input.ts'
  //   | 'socket-server.ts';
  export interface ScriptConfigResponseMap {
    'debug-server.ts': {
      config: DebugServerConfig;
      response: DebugServerResponse;
    };
    'debug-server.js': {
      config: DebugServerConfig;
      response: DebugServerResponse;
    ...
    'debug-server-cluster.ts': {
      config: DebugServerClusterConfig;
      response: DebugServerClusterResponse;
    };
    'socket-server.ts': {
      config: SocketServerConfig;
      response: SocketServerResponse;
    };
  }
  export type ScriptFileName = keyof ScriptConfigResponseMap;
}
