...
| `info` | daemon id 或 子进程 id | `{ type: 'info', data: DaemonInfo \| CpManagerInfo }` |
| `start` | `CpManagerConfig` 对象或子进程 id | `{ type: 'start', data: CpManagerInfo }` |
| `restart` | 同 `start` | `{ type: 'restart', data: CpManagerInfo }` |
| `stop` | daemon id 或子进程 id | `{ type: 'stop', data: DaemonInfo \| CpManagerInfo }` |
...
```typescript
{
  ...
  cpManagerConfigList?: CpManagerConfig[]; // 启动时自动拉起的子进程列表
}
```
